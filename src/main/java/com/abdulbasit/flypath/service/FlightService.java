...
public class FlightService {
    ...
    private final Amadeus amadeus = Amadeus.builder(dotenv.get("API_KEY"),dotenv.get("API_SECRET")).build();
...
//                                airlineService.getAirline(segment.getCarrierCode())
//                        );
//                        flights.add(flight);
//                    }
//                }
...
//    }
    ...
    public List<Itinerary> searchFlights(FlightRequest request) {
        logger.error("Cache missed hitting the API");
        ...
        try {
            ...
            for (FlightOfferSearch offer : flightOffers) {
                ...
                double totalPrice = Double.parseDouble(offer.getPrice().getTotal());
                myItinerary.setPrice(String.valueOf(currencyConversion.convertEurToPkr(totalPrice)));
